...
public class PoopTopology {
  ...
  public static ArrayList<IncidentBean> incidentBeans = new ArrayList<IncidentBean>();
  ...
  public static class IncidentSpout extends BaseRichSpout {
    ...
    public void nextTuple() {
        ...
        if(incidentBeans.size()>0){
            IncidentBean nextBean = incidentBeans.remove(0);
            _collector.emit(new Values(nextBean), nextBean);
        }else{
             try{
                Thread.sleep(10000);
                LOG.info("waiting for new incident data");
                incidentBeans = incidentComms.getIncidentUpdates();
                if(incidentBeans.size()>0){
                    LOG.info("NEW DATA - incidentBeans size:"+incidentBeans.size()+"");
                }
            }
            catch(Exception e){
                LOG.error("sleep failed");
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        ...
        declarer.declare(new Fields("IndcidentBean"));
    }
  }
...
//   }
  ...
  public static void main(String[] args) throws Exception {
    ...
    builder.setSpout("spout", new ArrivalSpout(), 1);

    builder.setBolt("AddExclaim", new AddExclaim(), 4).shuffleGrouping("spout");
   // builder.setBolt("count", new WordCount(), 4).fieldsGrouping("split", new Fields("word"));

    Config conf = new Config();
    conf.registerSerialization(ArrivalBean.class);
    conf.registerSerialization(IncidentBean.class);
